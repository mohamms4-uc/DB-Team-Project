...
app.get('/api/user/:userId', async (req, res) => {
    ...
    try {
        ...
        const userResult = await pool.query('SELECT user_name, password FROM app_user WHERE user_id = $1', [userId]);
        ...
        // Fetch addresses
        const addressResult = await pool.query('SELECT * FROM address WHERE address_id = (SELECT u_address_id FROM app_user WHERE user_id = $1)', [userId]);

        // Fetch credit cards
        const creditCardResult = await pool.query('SELECT * FROM creditcard WHERE c_user_id = $1', [userId]);

        // Send response with separate arrays
        res.json({
            user,
